...
Car.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle) {
    level.playerTouched(obstacle);
    if (keys.up && this.speed.y > 0 && jumpCounter == 0)
    {
  ...
  if (keys.up && this.speed.y > 0 && jumpCounter != 2)
  {
    this.speed.y = -dJumpSpeed;
    jumpCounter +=1;
  }
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "car" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
    ...
    if (!this.actors.some(function(actor) {
      ...
      this.status = "secretwon";
      this.finishDelay = 1;
      //Goes to a secret level if player gets all coins

    }
  }
  else if (type == "goal" && this.status == null)
  {
    this.status = "won";
    this.finishDelay = 1;
  }
  else if (type == "goalF" && this.status == null)
  {
    this.status = "finalwon";
    this.finishDelay = 1;
  }
};
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
        ...
        startLevel(1);
      else if (n <plans.length - 1 && status == "secretwon")
        startLevel(2);
      else if (n < plans.length -1 && status == "finalwon")
        console.log("You win! But did you find the secret level?");
    });
  }
  startLevel(0);
}
